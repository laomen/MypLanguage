...
| 语法 | `@coro` 注解（`has_coro` 标记，parser 已解析，作用于**类 action 方法**）+ `await` 语句/表达式（`AwaitStmt` + `AwaitExpr`，支持 `await;` / `await expr;` / `int v = await expr;`）|
| 标准库 | `stdlib/coro.myp`：FFI 声明（`create`/`set_entry`/`yield(val)`/`resume(h,val)`/`is_active`/`destroy`/`set_entry_arg`/`get_entry_arg`/`set_result`/`result`）|
...
| 无测试/示例 | ✅ `tests/coro/` 已建（C1+C2：spawn/恢复/参数/值传递/返回值/is_active/destroy/多协程）|
