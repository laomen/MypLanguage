...
| 3 | **一等函数 / 闭包** | lambda 语法在（`(x)=>{}`），但**无函数类型**，无法存储/传递/返回。**设计稿已就绪**（`docs/function.md`：函数类型 `(A,B)->R` + fat pointer 表示 + 按值捕获 + stdlib map/filter/reduce + Option.map；M-FN-1/2/3） | 高阶函数（map/filter/reduce 可落地） | 中 | P1 |
