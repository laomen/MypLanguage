...
| 3 | **一等函数 / 闭包** | 设计见 `docs/function.md`。**M-FN-1 已实施**：函数类型 `(A,B)->R` + lambda → 一等函数值（fat pointer `{closure, call_fn}` + 每 lambda tramp）+ 存/传/返/调用 + 函数类型参数登记。`tests/function`。**M-FN-2（捕获）M-FN-3（map/filter/Option.map）待实施** | 高阶函数（map/filter/reduce 可落地） | 中 | P1 |
