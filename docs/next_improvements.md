...
| 3 | **一等函数 / 闭包** | 设计见 `docs/function.md`。**M-FN-1/M-FN-2/M-FN-3 已实施**：函数类型 `(A,B)->R` + lambda 一等函数值（fat pointer `{closure, call_fn}` + tramp）+ 存/传/返/调用 + **按值捕获**（标量/字符串深拷贝、class 引用浅拷贝、嵌套、上下文类型推断）+ **`nonlocal` 按引用捕获**（共享可变 cell：`nonlocal k;` → 函数序言提升变量为堆 cell，lambda 读写直达；v1 标量类型、非嵌套 lambda、非 struct 方法；Man or Boy = -67 与 Go 参考一致）+ **泛型高阶函数**（`mapOpt`/`foldInt`：泛型参数 + 一等函数实参，泛型函数模板与类占位符延迟单态化）+ **`Option.map`** + **泛型 `@static` 类方法**（`List.map<T,R>` stdlib 落位）。`tests/function` + `tests/generic_static` + `tests/@test/nonlocal` + `tests/@test/man_or_boy` | 高阶函数（map/filter/reduce 可落地） | 中 | P1 |
...
> backing 级联释放，以及 **M1/M2/M3/M5/M6/M8/M9（内存系列）+ M7（weak 引用）**
> 均已实施（见下表 ✅），通过 O0/O2/ASan/UBSan/TSan 219/219。下表其余项仍未实施；
...
| ~~M6~~ | **~~跨线程 class 所有权~~** | ARC 计数当前非原子，语言约定对象线程本地；若 class 经事件、静态槽或 FFI 跨线程共享，retain/release 存在数据竞争 | 明确实现原子 ARC，或提供冻结后一次性 move 的线程转移语义；在方案落定前增加 sema 诊断，阻止可识别的 class 跨线程捕获/存储 | TSan 下多线程传递、并发 retain/release、最后持有者释放和线程提前退出无竞态；性能基准量化原子模式成本 | ✅ **已完成**（原子 ARC：`myp_obj_header_t.rc`/`myp_arr_header_t.rc` 改 `_Atomic uint32_t`，retain 用 relaxed 原子自增、release 用 release-acquire 递减 + 最后持有者 acquire fence 后释放；**分配列表改进程全局 spinlock 保护**——跨线程释放安全（旧 TLS 列表：node 的 prev/next 指向别的线程列表 → 跨线程 free 损坏列表 + 分配线程退出双重释放），列表仅进程退出（atexit，先协程清理后全局释放）释放、线程退出不再释放。`tests/stress/cross_thread_arc`：4 线程并发 retain/release 共享实例（TSan 无 race；非原子版本复现 race）+ 跨线程最后持有者释放 + 释放后继续分配；217/217（O0/O2/ASan/UBSan）、TSan 2/2、ASAN stress 6/6；成本量化：500k 类分配 10ms、字符串/基准与修复前持平（≤2%）） |
| ~~M7~~ | **~~循环引用与 weak~~** | 纯 ARC 无法回收强环；当前仅文档约定组件图为 DAG，双向 parent/child、回调捕获 self 等模式仍可形成永久泄漏 | 设计 additive `weak` 引用：对象销毁时置空观察者，不增加强计数；先覆盖 class 字段和闭包捕获，再评估数组/接口 weak 槽。静态可疑环只做警告，不代替运行时语义 | 双节点环、self 环、闭包环在断开强引用后 live count 回零；weak 读取在对象释放后得到 null；并发语义与 M6 一并定义 | ✅ **已完成**（`@weak` 字段注解——class/接口引用字段；写入 `myp_weak_store`（不计数）、读取 `myp_weak_load`（弱→强升级返回强引用，调用方释放，`isFreshArcExpr`/返回消费已接入）、销毁桩 `myp_weak_clear` 注销+置空。**全局 weak 登记表**（chained-hash，spinlock，与 M6 同模型）：对象 rc→0 时 `myp_weak_notify_death` 先置空所有观察槽再释放（弱锁内复查 rc，处理并发升级竞态），atexit 释放登记表。**附带修复**：if/while/for 条件求值产生的临时（class-returning call / weak 升级）被分支体语句末 flush 在条件块内抢先释放、另一路径泄漏——新增 `arcReleaseConditionTemps` 在条件求值后立即释放。`tests/weak_cycle`（双向环/自环/观察者断开回零 + 销毁后读 null）+ `cross_thread_arc` 加 weak 并发场景；219/219（O0/O2/ASan/UBSan）、TSan 2/2、ASAN stress 6/6） |
...
19. ✅ **M6 跨线程原子 ARC**：`myp_obj_header_t.rc`/`myp_arr_header_t.rc` 改 `_Atomic uint32_t`（retain relaxed 自增、release release-acquire 递减 + 最后持有者 acquire fence）；分配列表改进程全局 spinlock（跨线程释放安全，旧 TLS 列表跨线程 free 会损坏），仅进程退出 atexit 释放。`tests/stress/cross_thread_arc`（4 线程并发 retain/release + 跨线程最后释放 + 释放后分配；TSan 无 race、非原子复现 race）。成本 ≤2%（500k 类分配 10ms、基准持平）
20. ✅ **M7 weak 引用**：`@weak` 字段注解（class/接口引用字段）——写入 `myp_weak_store`（不计数）、读取 `myp_weak_load`（弱→强升级，返回强引用，`isFreshArcExpr`/返回消费接入）、销毁桩 `myp_weak_clear` 注销+置空。全局 chained-hash weak 登记表（spinlock，与 M6 同模型）：rc→0 时先置空所有观察槽再释放（锁内复查 rc 处理并发升级）。**附带修复**：if/while/for 条件临时被分支体 flush 抢释、另一路径泄漏（新增 `arcReleaseConditionTemps`）。`tests/weak_cycle`（双向环/自环/观察者回零 + 销毁后读 null）+ cross_thread_arc 加 weak 并发；219/219、TSan 2/2、ASAN stress 6/6
