...
清单划掉——其核心前提在 M8 计数头落地后不再成立：

- M4 前提：`@region` 函数含任一逃逸分配 → 整函数禁用 region → 同函数内大量不逃逸
  临时值"活到线程退出"。该前提写于 M8 之前；M8 计数头回收后，禁用路径已从"arena
  活到线程退出"变为**计数路径 rc→0 即释放**。
- 探针实测（2026-09-07，可变长 `int[]` 避开栈提升）：@region 内 200 万次不逃逸
  分配，PURE（region 快路径）与 ESC（含 1 个逃逸 return → region 禁用）均 18ms；
  ESC 经 10×20 万次调用 `arenaReservedBytes` delta=0（零累积，临时随作用域即刻
  释放）。RSS 与吞吐两维均无差异 → 逐分配点数据流（return/PHI/别名）+ oracle/
  selfhost 双端实现成本无回报。
- 与既有实测互证：栈逃逸分析（v3.15.181-185）同样显示函数级保守分析对真实混合
  负载零收益。剩余真实瓶颈为 ARC 调用点 codegen（跨模块小函数未内联 +
  retain/release 簿记）与 string 拼接分配数，M4 不触及。
- M8 设计注本即"优先依靠 M4…若仍是主因再引入计数头"——实现实际选了 M8 通用路线
  （全计数头回收，RSS 稳定）后 M4 冗余。docs 本地文件（gitignored）三处已划。
