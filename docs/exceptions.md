...
ThrowStmt    ::= 'throw' Expression? ';'                      // 无表达式 = 在 catch 内重抛
...
| E4.11 | 标准异常：`FileError`/`IOError`/`NetError`/`ParseError`/`JsonError`/`ArgumentError`/`MathError`/`IndexError`（`stdlib/error.myp`，全部实现 `Error`，可 `catch (Error e)` 统一捕获）| ✅ |
| E5 | 测试 `tests/exception/` 扩展（类型分发/传播/未处理/表达式 try/重抛/标准异常/库接入/线程）+ grammar.md 增量 | ✅ 29 用例 + `exception_lib`/`exception_thread`（正常 + ASAN 91/91）|
