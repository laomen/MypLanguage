...
> 状态：**核心已实施（v3.3/v3.7）**——`--passes myp-pass` 自定义优化 pass（v3.3）+
> `-O1/-O2/-O3` IR 优化管线 + `-g` DWARF 调试信息（v3.7，配合 `myp_debug`/gdb）。
...
| **M2** | 优化 × 异常/协程兼容专项回归 + 修复 | ✅ 已完成（`tests/exception*`、`tests/coro*` 在 -O2 专项回归全部通过；性能基准：`-O2` 比 `-O0` 至少 45×（45ms→<1ms），结果一致；`-O0`/`-O2` 全套 109/109）。**追加（§五-3 异常 × -O2 修复，套件涨到 173 后首次复验）**：`-O2` 暴露 `result` 段错误 + `arc_throw` 泄漏——异常 dispatch/propagate 读 try 内 ARC 槽时，其唯一 def 在 try_block（不支配 longjmp 路径），LLVM 把 load 折叠成 `undef`（`MYPC_DUMP_OPT_IR=1` 见 `call myp_release(ptr undef)`）。修复：`myp_release_slot(槽地址, kind)` 在运行时读**物理槽位**再释放（对 LLVM 不透明）+ `registerArcSlot` 内 `myp_try_escape` 让槽逃逸保住 try_block 的 store；协程帧镜像仍先 `emitCoroFrameClear`。另修 `mypc run` 不支持子命令前 flag（`-O2 run`）+ 5 个自举子脚本 `"$MYPCC"` 引号在 `MYPCC` 带参数时失效。`-O0`/`-O2`/ASAN 全套 173/173 |
