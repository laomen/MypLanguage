...
#### 当前实现（codegen，函数级保守分析）

1. 收集 `@region` 函数内的 slice/数组局部变量。
2. 检测 return、property/global store、subscript store、throw 和调用实参等逃逸出口。
3. 未发现引用逃逸时建立 arena mark，并在所有正常出口回滚。
4. 发现引用逃逸或函数直接返回引用时，保守禁用该函数的 region，全部分配保持持久。

该策略首先保证 100% 不悬垂；代价是同一函数中即使只有一个对象逃逸，其他局部临时也
不会 region 回收。后续可在 R2 benchmark 证明收益后升级为持有者图 + 逐分配提升，不影响
现有语义。局部-only 的链式/帧处理场景仍使用 chunk 回滚快速路径。
