...
4. **管道 `|>`**：需要确定算子组件如何作为管道节点（实例 vs 类名 vs 函数名）
...
| 阶段 | 内容 | 风险 |
|---|---|---|
| P1 | 顶层 `@op` 函数（parser 注解 + sema 注册 + 二元分发 + codegen 调用） | 中 |
| P2 | struct `operator:` 节（parser + sema + codegen） | 中 |
| P3 | `|>` 管道 token + 表达式 + 组件节点 | 中 |
| P4 | 元素级提升 + 集合二元 | 高 |
| 贯穿 | 示例（Vector3 / Set）+ 回归测试 + grammar.md 增量 + CHANGELOG | — |
