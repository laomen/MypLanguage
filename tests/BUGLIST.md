...
## BUG-003（未修复）：泛型 T=string 的 `<`/`>` 按指针比较

- **状态**：🟥 未修复
- **复现**：`tests/bugs/generic_string_cmp.myp`
- **现象**：泛型实例化为 string 时，`<`/`>` 未走 `myp_str_cmp`（按内容），退化
  为指针地址比较 → 同一逻辑在不同文件/字面量布局下结果不同（`generic_traits`
  回归 `s=a` 实得 `s=b` 即此因）。对照：非泛型 string 比较恒正确。
- **验证**：修复后 `tests/bugs/generic_string_cmp.myp` 转绿，`tests/generic_traits` 回归同时转绿。
