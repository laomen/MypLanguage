...
- **现象**：myp_http 协程服务端（serveCoro）向已断开客户端写响应 → 进程 SIGPIPE 崩（exit 141「断开的管道」）。
  ...
  曾误判为"10+ 慢挂起连接高并发饿死"（0/20 秒失败）——实为服务端已崩（进程消失），非调度问题。
- **位置（运行时三处 send flags=0，均无 MSG_NOSIGNAL）**：
  - `runtime_myp/net.myp` `myp_net_send`：`send(fd, ptr, len, 0)`
  - `runtime_myp/uds.myp` sendto：`__myp_syscall(44, fd, buf, len, 0, 0, 0)`（flags=0）
...
## BUG-151（已修复 🟩，v3.15.232）：@parallel 并发 worker 异常状态跨线程串扰 → 段错误/假 uncaught

- **现象**：@parallel 并发 worker 内 try/catch/throw（如 mypagent `fan_shared_mem_check`
  真并发子 agent 各 tick 内 try/catch + Json/memory 操作）间歇崩——隔离 ~5-7%
  `uncaught exception (object, type 21)` exit 134（abort），或段错误 exit 139 /
  总线错误 exit 135。type 21 = JsonError：子 agent tick 内 Json 解析失败**本应被
  同线程 catch**，却报未捕获（假 uncaught，实为异常状态被并发破坏）。
- **根因（MYP runtime 落后于 C runtime）**：异常运行时状态 `Exc`（depth/handlerBufs/
  curType/curObj/errBuf）是 `@static class` → **进程级全局、非线程安全**；C runtime
  （runtime.c）同函数是 `static __thread`（真 TLS）→ C 版无此 bug（selfhost/MYP
  runtime 独占缺陷）。@parallel worker 是 pthread（v3.15.77 起 myp_thread_spawn 用
  pthread_create 建真 TLS），各自 try/catch 的 push/pop **交叉读写共享 handler 栈**：
  ①worker throw → myp_exception_get_jmpbuf 读全局 depth 顶部槽 = **别线程**刚 push
  的 jmp_buf → longjmp 到别线程栈帧 → 段错误/总线错误；②并发 push/pop 非原子 →
  depth 丢失到 0 → 本应同线程 catch 的异常报 "uncaught" → exit(134) 杀全进程。
- **最小纯 MYP 复现（BUGLIST ③，本版建立）**：`@parallel for` 8 worker × 40000 项 ×
  16 次内层 `try{ (i+k)%5==0 → throw new Boom(); } catch(Boom/Error/e)`——修复前
  ~7/8 段错误/总线错误；no-throw 变体全稳定（隔离到 throw/longjmp 路径）；
  arena 预暖不变（排除分配首触）。mypagent fan_shared_mem_check 修复前隔离
  2/25 崩。
- **修复（v3.15.232，runtime_myp/exception.myp 一处）**：`Exc` 改 `@static @thread
  class` → LLVM `thread_local`（@thread 静态类自 v3.15.77 pthread TLS 起真逐线程）
  ——异常状态逐线程，try/catch/throw 各自线程内一致（同线程 catch 生效、无跨线程
  longjmp、无假 uncaught），与 C runtime `__thread` 语义对齐。
...
- **现象（mypagent `HttpServer.serveCoro`，v0.1.138）**：
  ...
  - **真并发突发楔死（关键，与 handler 内容无关）**：2 个 curl **同一瞬间**并发 GET →
    请求全超时（curl `--max-time` code 000）；服务端进程**存活**（非 exit 141/139）。
  - **隔离结论（v0.1.138 补测，推翻 fork 假设）**：**纯 echo handler（无 fork/文件 IO/鉴权）
    同样楔死**——自包含最小复现（stdlib 内联同款 acceptLoop/connCoro，回 "ok"）2 路同一瞬间
    并发 GET，1-2 轮即楔（3/3 稳定）；突发后单发也挂（服务端存活）。→ 是反应堆 accept/fd
    就绪处理缺陷，非 handler（Process.output fork/文件 IO）因素。
  - 突发后遗症：楔死后单发也全超时——acceptor 不再被就绪处理/poll。
- **位置**：`runtime_myp/coro.myp`（selfhost/MYP runtime：Coro.scheduler/waitFd、fd 注册与
  ...
  connCoro（mypagent）与 tests/bugs/b153_reactor_wedge/srv.myp（自包含内联同款）行为正确，仅暴露
  此运行时边界。
- **根因（未定位，待查）**：2 路**同一瞬间**连接时（两 fd 同批就绪），acceptor `waitFd` 循环与
  新 spawn 的 connCoro 就绪处理边界——疑似 accept 后监听 fd 的 poll 注册/就绪状态未正确重建，
  或同批就绪多 fd 只消费一个后丢失。需 runtime_myp 调度器深挖（poll 集合维护 + 每轮就绪驱动）。
  已排除 handler/fork/文件 IO 因素（纯 echo 亦楔）。
...
- **复现（自包含，3/3 稳定 1-2 轮即楔）**：`tests/bugs/b153_reactor_wedge/{srv.myp,repro.sh}`——
  srv.myp 纯 stdlib 内联同款反应堆（acceptLoop+connCoro，回 "ok"，无 fork/文件 IO）；
  repro.sh 每轮 2 路同一瞬间并发 GET + 1 路跟进单发，任一非 200 判楔。
  运行：`cd tests/bugs/b153_reactor_wedge && bash repro.sh [port] [rounds]`（退出 0=复现楔死）。
  网关侧现场：mypagent `tests/bug153_repro.sh`（examples/b153_repro_server.myp，同款 echo）。
