...
- `sha256`（0.56）是唯一剩余大差距，根因是 **MYP 没有 32 位无符号类型**，用
  `long` 模拟 uint32：①LLVM 认不出 i64 上的 32 位旋转 → 6 个 rotate 各 3 条
  （shr+shl+or）而非原生 `ror`；②每轮 `&0xFFFFFFFF` 截断 + 8 个状态变量溢出到栈
  （g++ 的 u64 模拟版只有 15 条移位且状态全在寄存器，20ms vs MYP 32ms）。加 `u32`
  类型是正解（届时 LLVM 会发射 `rorl`、32 位寄存器、无 mask，可到 ~0.9+）。
