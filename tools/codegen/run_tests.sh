...
# MYPCC 可带参数（如 "./build/mypc -O2"，run_tests_O2.sh 用）：二进制取首个词
# 绝对化（相对路径以调用方 cwd 为基准），flags 保留，再 cd 进脚本目录——否则
# 相对 MYPCC（如 ./build/mypc）在 cd 后解析失败。
MYP_ABS=${MYPCC%% *}
case "$MYP_ABS" in
    /*) : ;;
    *) MYP_ABS="$(cd "$(dirname "$MYP_ABS")" && pwd)/$(basename "$MYP_ABS")" ;;
esac
MYPCC="$MYP_ABS${MYPCC#${MYPCC%% *}}"   # 二进制绝对化 + 保留 flags
cd "$(dirname "$0")"

if [ ! -x "$MYP_ABS" ]; then echo "error: mypc 不存在 ($MYPCC)"; exit 1; fi
export MYP_CC="$MYP_ABS"   # 让 main.myp --verify 的子进程定位到同一编译器
