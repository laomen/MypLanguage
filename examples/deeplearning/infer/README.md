...
//   ——层式 JSON 描述网络（Gemm/Conv/Pool/Relu/Softmax + 权重 init 或
//      safetensors 源 W.safetensors{file,tensor} 直接读值）构建 Graph
//   ——权重 B 可选（无 bias Gemm 支持）
```
...
../infer_tests/        # 端到端回归（70 个 *_main.myp，见 README.md）
...
bash /tmp/run_infer_tests.sh    # → == pass=70 fail=0 ==
